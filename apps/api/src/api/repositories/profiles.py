...
def _profile_page(response: APIResponse, *, page: int, page_size: int) -> Page[ProfileRecord]:
    return Page[ProfileRecord](
        items=[_hydrate_profile(row) for row in response.data],
        page=page,
        page_size=page_size,
...
def _build_profile_upsert_row(
    ...
    *,
    ...
    role = existing_profile.role if existing_profile is not None else Role.member
...
def get_actor(user_id: UUID, *, client: Client | None = None) -> ProfileRecord | None:
    return _profile_by_id(_resolve_client(client), user_id)
...
def upsert_onboarding(
    ...
    *,
    ...
    profile_row = _build_profile_upsert_row(payload, existing_profile=existing_profile)

    profile_response = resolved_client.table("profiles").upsert(profile_row, on_conflict="id").execute()
    profile = _hydrate_profile(profile_response.data[0])

    approval_request_response = (
    ...
    _hydrate_approval_request(approval_request_response.data[0])
    return profile
...
def list_profiles(
    *,
    ...
    return _profile_page(response, page=page, page_size=page_size)
